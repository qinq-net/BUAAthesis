...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{./resource/EventDisplay.png}
...
径迹测量主要包括利用TPC和MWDC对粒子径迹的捕捉。以下分别说明本框架对TPC和MWDC的
数字化输出和分析方法。
...
随后，本框架计算了每个
记录单元应当产生信号的强度和时间，并产生类似代码\ref{data:TPC}的输出（截取部分）：

\begin{lstlisting}[caption={TPC Cells},label={data:TPC},firstnumber=3384,lastline=3387]
...
\begin{figure}
	...
	\caption{TPC的输出结果，方框为TPC的灵敏区域}\label{fig:result:TPC}
\end{figure}
...
\end{equation}
得到$y=\SI{8}{\milli\meter}$。模拟详细数据如代码\ref{data:MRPC:85-1}所示
可以看到，Y坐标确实在\SI{8}{\milli\meter}左右：
...
%% 以下用于截去附录
%\makeatletter
%	\def\@currentlabel{B}\label{chap:data}
%	\def\@currentlabel{B.1}\label{data:TPC:Track}
%	\def\@currentlabel{B.2}\label{data:MRPC}
%\makeatother
%\end{document}
% 附录
\appendix
