...
例如，样例事件首先可以得到类似附录\ref{chapter:data}中代码\ref{data:TPC:Track}
的针对每个次级电子输出。
...
\begin{lstlisting}[caption={eTOF1},firstnumber=4406,lastline=4453]
...
\chapter{示例数据}\label{chapter:data}
...
以下是TPC记录的跟踪数据的截取部分的样例。其中每行记录了一个在TPC内进入漂移的次级电子
